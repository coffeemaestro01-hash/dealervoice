    ...
    <section className="py-16 bg-white">
      <div className="container">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-600 mt-2">Find the right dealership for whatever you&apos;re looking for.</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {CATEGORIES.map((c) => (
            <Link
              key={c.label}
              href={c.href}
              className="group flex flex-col items-center text-center gap-3 rounded-2xl border border-gray-200 bg-white p-6 hover:border-gold/50 hover:shadow-lg hover:shadow-gold-100/50 transition-all"
            >
              <span className="grid place-items-center w-14 h-14 rounded-2xl bg-gold-50 text-gold-600 group-hover:bg-gold-gradient group-hover:text-night-900 transition-colors">
                <c.icon size={26} />
              </span>
              <span className="font-semibold text-gray-800 group-hover:text-gold-700">{c.label}</span>
            </Link>
          ))}
        </div>
      </div>
