...
${templateHint}`;
  ...
  try {
    ...
    return { subject: `Re: ${input.ticketSubject}`, body: raw };
  }
}
...
Ask one clear question at a time. Provide concrete DNS/forwarding steps when relevant.
If setup appears complete, set nextStep to "verify" and summarize what to test.
...
user: ${input.userMessage}`;
  ...
  try {
    ...
    return {
      reply: "Tell me which email system you use today (Gmail, Outlook, or other) and the support address customers should email.",
      nextStep: input.step,
