...
export default async function AdminPaymentsPage() {
  ...
  return (
    <div className="p-6 lg:p-8 space-y-8">
      <h1 className="text-2xl font-bold text-foreground">Payments & webhooks</h1>
      <p className="text-sm text-muted-foreground">Stripe is the sole payment provider for subscriptions and platform billing.</p>

      <section>
        <h2 className="font-semibold mb-3">Stripe webhook log</h2>
        <div className="bg-card rounded-xl border divide-y max-h-96 overflow-y-auto">
          {webhooks.length === 0 ? (
            <p className="p-6 text-muted-foreground text-sm">No webhook events yet.</p>
          ) : (
            webhooks.map((w) => (
              <div key={w.id} className="px-4 py-2 text-sm flex justify-between gap-4">
                ...
                <span className="text-muted-foreground text-xs">{new Date(w.createdAt).toLocaleString()}</span>
              </div>
            ))
          )}
        </div>
