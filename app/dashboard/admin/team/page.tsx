  ...
  return (
    <div className="p-6 lg:p-8 space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Prime team split for DealerVoice.io operations.</p>
      </div>

      <div className="grid md:grid-cols-2 gap-4">
        {TEAM_MATRIX.map((t) => (
          <div key={t.team} className="bg-white rounded-xl border p-5">
            <h2 className="font-semibold text-gray-900">{t.team}</h2>
            <p className="text-sm text-gray-600 mt-2">{t.areas}</p>
          </div>
        ))}
      </div>
      ...
      <p className="text-sm">
        Assign roles in{" "}
        <Link href="/dashboard/admin/users" className="text-gold-700 hover:underline">
