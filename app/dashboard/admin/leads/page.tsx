...
export default async function AdminLeadsPage() {
  ...
  return (
    <div className="p-6 lg:p-8">
      <h1 className="text-2xl font-bold text-gray-900 mb-2">Global lead inbox</h1>
      <p className="text-sm text-gray-500 mb-6">All customer leads across dealerships.</p>
      <div className="bg-white rounded-xl border divide-y">
        {leads.length === 0 ? (
          <p className="p-6 text-gray-500 text-sm">No leads yet.</p>
        ) : (
          leads.map((l) => (
            <div key={l.id} className="px-4 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <p className="font-medium text-gray-900">{l.name} · {l.email}</p>
                <p className="text-sm text-gray-600">{l.dealership.name} · {l.type}</p>
